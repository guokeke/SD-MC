...
int PowerStateIsAC and skip;
 ...
 function main ( int RValue )
 {
	 ...
	 temp_WarmPollPeriod, temp_status, temp_polling, temp_PowerStateIsAC, end) and (
	 int end and skip;
     ...
     if(main_1_rand_x!=0) then 
     {
         ...
         if(main_1_5_rand_x1!=0) then 
         {
			 ...
			 int temp_WarmPollPeriod and skip;
             int temp_status and skip;
             int temp_polling and skip;
             int temp_PowerStateIsAC and skip;

             while( break$=0 AND   ( count$=0 OR 1))
             {
			     if(temp_WarmPollPeriod=WarmPollPeriod AND temp_status=status AND temp_polling=polling AND temp_PowerStateIsAC=PowerStateIsAC)then{
				    end:=1
				 }
				 else{
				   skip
				 ...
				 temp_WarmPollPeriod:=WarmPollPeriod;
				 temp_status:=status;
				 temp_polling:=polling;
				 temp_PowerStateIsAC:=PowerStateIsAC;
