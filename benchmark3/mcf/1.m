frame(syf,basket_size,basket1,perm,net,nr_group,group_pos,initialize) and (
int syf <==0 and skip;
struct node {
int number and 
char *ident and 
...
network net and skip;
  ...
  function main ( int RValue )
 {
     ...
     net.bigM:=(int)1.0e7;
     strcpy(net.inputfile,"F:/inp.in\0") and skip;
     if(extern read_min(&net,RValue)) then 
     {
  ...
  function read_min ( network *net,int RValue )
 {
     ...
     if(return=0)   then 
     {
         ...
         if(return=0)   then 
         {
             ...
             net->max_m:=3014656;
             net->nodes:=(node *)calloc(net->n+1,sizeof(node),RValue);
             net->dummy_arcs:=(arc *)calloc(net->n,sizeof(arc),RValue);
             net->arcs:=(arc *)calloc(net->max_m,sizeof(arc),RValue);
