...
class CoreComponentsFactory {
  ...
  @Option(description="use PoinerCheckalgorithm to  validate a null pointer problem")
  private boolean usePoinerCheckAlgorithm = false;
  @Option(description="use SimplePropertyCheckAlgorithm to  validate a property")
  private boolean useSimplePropertyCheckAlgorithm = false;
  @Option(description="use ArrayCheckAlgorithm to  validate a boundOfArray problem")
  private boolean useArrayCheckAlgorithm = false;
  ...
  public void print(){
    System.out.println(useProofCheckAlgorithm);
    System.out.println(useRestartingAlgorithm);
    System.out.println(useImpactAlgorithm);
    System.out.println(useRefinement);
    System.out.println(useBMC);
    System.out.println(useCBMC);
    System.out.println(useFeatureVarsRestriction);
    System.out.println(useAssumptionCollector);
    System.out.println(useAdjustableConditions);
  }
  public Algorithm createAlgorithm(final ConfigurableProgramAnalysis cpa,
    ...
    }else if(useSimplePropertyCheckAlgorithm){
      algorithm = new SimplePropertyCheckAlgorithm(config,cpa,logger);
    }else if(usePoinerCheckAlgorithm){
      algorithm = new PointerCheckAlgorithm(config,cpa,logger);
    }else if(useArrayCheckAlgorithm){
      algorithm = new ArrayCheckAlgorithm(config,cpa,logger);
    }else{
      algorithm = new CPAAlgorithm(cpa, logger, config);
     // System.out.println("算法:"+algorithm.getClass().toString());
      if (useRefinement) {
  ...
  public ReachedSet createReachedSet() {//see9
