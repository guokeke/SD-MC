...
public class CFACreator {
  ...
  public CFA parseFileAndCreateCFA(String filename)
    ...
    try {
      ...
      ParseResult c = parser.parseFile(filename);//see1
      /*ParseResult myString=parser.parseString("void main(){i==j;j<=k;}");
      SortedSetMultimap<String, CFANode> sm=myString.getCFANodes();
      Collection<CFANode> sl=sm.values();
      for(CFANode cn:sl){
        List<CFAEdge> ce=cn.getLeavingEdge();
        for(CFAEdge e:ce){
     ...
     //
     /* System.out.println("-------------immutableCFA------------");
      SortedSet<CFANode> vset=immutableCFA.getAllNodes();
      Iterator<CFANode> it=vset.iterator();
      while(it.hasNext()){
        CFANode cnode=it.next();
        System.out.println(cnode.getNodeNumber()+":"+cnode.getLineNumber()+"===>"+cnode.getLeavingEdge());
      }*/
      //
      return immutableCFA;
