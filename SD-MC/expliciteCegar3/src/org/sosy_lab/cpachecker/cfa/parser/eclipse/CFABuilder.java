...
class CFABuilder extends ASTVisitor {
  ...
  public void printCFAInf(){//mycode
      System.out.println("*******CFA*******");
      System.out.println("cfas=>"+cfas);
      System.out.println("functionDeclarations=>"+functionDeclarations.toString());
      Set<String> set=cfaNodes.keySet();
      Iterator<String> iterator=set.iterator();
      while(iterator.hasNext()){
      String next= iterator.next();
      SortedSet<CFANode> vset=cfaNodes.get(next);
      Iterator<CFANode> it=vset.iterator();
      while(it.hasNext()){
        CFANode cnode=it.next();
        System.out.println(cnode.getNodeNumber()+":"+cnode.getLineNumber()+"===>"+cnode.getLeavingSummaryEdge());
      }
      System.out.println("cfaNodesMap=>"+next+","+cfaNodes.get(next));
      }
