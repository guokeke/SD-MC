...
public class SSAMap implements Serializable {
  ...
  public static class SSAMapBuilder {
    ...
    public Multiset<String> getSSAMap(){
      return ssa.vars;
    }
    ...
    public void setIndex(String var, int idx) {
      ...
      //System.out.println(idx+" "+var+"="+varsBuilder.count(var));
    ...
    public void addVars(String function) {
      // TODO Auto-generated method stub

      Multiset<String> vars=ssa.vars;
      Iterator<String> it=SSAMapInfo.getInstance().getVars().elementSet().iterator();
      Set<String> set=vars.elementSet();
      while(it.hasNext()){
        String next=it.next();
        if(next.contains("::")){
          String func=next.substring(0,next.indexOf("::"));
          int count1=SSAMapInfo.getInstance().getVars().count(next);
          if(!set.contains(next)&&func.contains(function)){
            vars.add(next);
    ...
    public void removeVars(String function) {
      // TODO Auto-generated method stub
      Multiset<String> vars=ssa.vars;
      Iterator<String> it=SSAMapInfo.getInstance().getVars().elementSet().iterator();
      Set<String> set=vars.elementSet();
    ...
    public void setVars(Multiset<String> pRemain) {
      // TODO Auto-generated method stub
      ssa.vars=pRemain;
    }
  }
   ...
   * SSAMap只保留所在函数的变量
   */
  public void removeOtherFunction(String function){
      if(!function.equals("null")){
          Multiset<String> remain=HashMultiset.create();
          Multiset<String> remove=HashMultiset.create();
          Iterator<String> iterator=vars.elementSet().iterator();
          while(iterator.hasNext()){
            String next=iterator.next();
            if(next.contains("::")&&!SSAMapInfo.predicates.contains(next)){
              String func=next.substring(0,next.indexOf("::"));
              if(!func.equals(function)){
                remove.add(next);
                remove.setCount(next,vars.count(next));
              }
              else{
                remain.add(next);
                remain.setCount(next,vars.count(next));
          ...
          SSAMapInfo.getInstance().setVars(remove);
      }
  }
  ...
  public void addVars(String function) {
    // TODO Auto-generated method stub
    Multiset<String> vars=this.vars;
    Iterator<String> it=SSAMapInfo.getInstance().getVars().elementSet().iterator();
    Set<String> set=vars.elementSet();
    while(it.hasNext()){
      String next=it.next();
