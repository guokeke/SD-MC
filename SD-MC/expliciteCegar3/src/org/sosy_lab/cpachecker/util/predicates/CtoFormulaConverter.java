...
public class CtoFormulaConverter {
  ...
  private Set<String> nondetFunctions = ImmutableSet.of(
      ...
      "__VERIFIER_nondet_short", "__VERIFIER_nondet_char", "__VERIFIER_nondet_float", "nondet","_VERIFIER_assume"
      );
  ...
  private static final Set<String> PURE_EXTERNAL_FUNCTIONS = ImmutableSet.of("__assert_fail", "free", "kfree",
  ...
  private static final Map<String, String> UNSUPPORTED_FUNCTIONS = ImmutableMap.of("pthread_create", "threads");
  ...
  private static final Predicate<CharSequence> IS_POINTER_VARIABLE = Predicates.containsPattern("\\Q"
      + POINTER_VARIABLE + "\\E.*\\Q__end\\E");
  ...
  public CtoFormulaConverter(Configuration config, ExtendedFormulaManager fmgr, LogManager logger)
  ...
  private static String scopedIfNecessary(CFAEdge edge, CIdExpression var, String function) {
    CSimpleDeclaration decl = var.getDeclaration();
    boolean isGlobal = false;
    ...
    if (edge instanceof CStatementEdge) {
      String leftEdge =
          ((CUnaryExpression) (((CExpressionAssignmentStatement) (((CStatementEdge) edge).getStatement()))
              .getLeftHandSide())).getOperand().toASTString();
      String leftEdgeRemSpace = leftEdge.replaceAll(" ", "");

      String preno = leftEdgeRemSpace.replaceAll(name, "0");
      CRLGlobal.getInstance();
      CRLGlobal.nostr = "(" + preno + ")";
      //noint=Integer.valueOf(preno);
  ...
  */
  ...
  private static String exprToVarName(CExpression e) {
    //龙---速度问题：不一致问题：a[1]与*(a+1)----把main::a[1]的var变成*(a+1)来处理
    //判断exp，如果为CArraySubscriptExpression，刨除结构体相关数组
    if (e instanceof CArraySubscriptExpression) {
      //System.out.println(e.toASTString());
      String myVar0 = e.toASTString().replaceAll("[ \n\t]", "");
      String myVar1 = myVar0.replaceFirst("\\[", "\\+");
      String myVar2 = myVar1.replaceFirst("\\]", "");
      return myVar2;
      //结束龙判断
    } else {
  ...
  private Formula makeVariable(String varName, SSAMapBuilder ssa) {
    if(Operators.isLeft&&KeyVar.keyVarSet.contains(varName)){
      Operators.curKeyVar=varName;
    }
  ...
  public PathFormula makeAnd(PathFormula oldFormula, CFAEdge edge)
    ...
    //  MyInfo.function=edge.getSuccessor().getFunctionName();
   ...
   // oldFormula.getSsa().addVars(function);
    //Multiset v=    SSAMapInfo.getInstance().getVars();
   if(MyInfo.function.equals("")){
     MyInfo.function=function;
   }
   if (edge instanceof CFunctionCallEdge) {
      oldFormula.getSsa().addVars(edge.getSuccessor().getFunctionName());
     //oldFormula.getSsa().addVars(function);
   }

    SSAMapBuilder ssa = oldFormula.getSsa().builder();//oldFormula的ssa：[std2, std, pE, main::a, main::k, main::w, main::d, main::i, main::p]===>加入了varsBuilder，是空的
    Constraints constraints = new Constraints();

    Formula edgeFormula = createFormulaForEdge(edge, function, ssa, constraints);//(and (= |set::b@2| |main::w@2|) (= |set::m@2| |__address_of__main::k@1|))
    ...
    if (useNondetFlags) {
      ...
      if (lNondetIndex != lFlagIndex) {
        ...
        for (int lIndex = lFlagIndex + 1; lIndex <= lNondetIndex; lIndex++) {
          Formula lAssignment =
              fmgr.makeAssignment(fmgr.makeVariable(NONDET_FLAG_VARIABLE, lIndex), fmgr.makeNumber(1));
    ...
    //龙：constraints：(ite (= |set::m@2| |__address_of__main::k@1|) (= |main::k@4| 6.0) (= |main::k@4| |main::k@3|))
    edgeFormula = fmgr.makeAnd(edgeFormula, constraints.get());//加入(not (= |__address_of__main::k@1| 0.0))赋值语句：加入(ite (= |set::m@2| |__address_of__main::k@1|) (= |main::k@4| 6.0) (= |main::k@4| |main::k@3|)))

    SSAMap newSsa = ssa.build();//{std2=2, std=2, pE=3, main::a=2, main::k=3, main::w=2, main::d=2, main::i=2, main::p=2, set::b=2, __address_of__main::k=1, set::m=2}
    ...
    //ssa.setVars(remain);
    String f=MyInfo.function;
    if(!MyInfo.function.equals(function)){
    MyInfo.changeFunction=MyInfo.changeFunction.equals("")?function:MyInfo.changeFunction;
    newSsa.removeOtherFunction(function);
    if (edge instanceof CFunctionCallEdge) {
      newSsa.addVars(edge.getSuccessor().getFunctionName());
    }
    }
  ...
  public PathFormula makeAnd(PathFormula oldFormula, CFAEdge edge, String fun)
      throws CPATransferException {
    // this is where the "meat" is... We have to parse the statement
    // attached to the edge, and convert it to the appropriate formula
   ...
   * @throws CPATransferException
   */
  private Formula createFormulaForEdge(CFAEdge edge, String function, SSAMapBuilder ssa, Constraints constraints)
      throws CPATransferException {
    ...
    case DeclarationEdge: {
      ...
      Operators.curKeyVar="";
      Operators.isLeft=false;
      return makeDeclaration(d, function, ssa, constraints);
    }

    case AssumeEdge: {
      Operators.isLeft=true;
      Formula result = makeAssume((CAssumeEdge) edge, function, ssa, constraints);
      Operators.isLeft=false;
      return result;
    }
    ...
    case FunctionReturnEdge: {
      ...
      Formula result = makeExitFunction(ce, function, ssa, constraints);
      Operators.isLeft=false;
      return result;
    }
  ...
  private Formula makeDeclaration(
    ...
    if (init == null) { return fmgr.makeTrue(); }
  ...
  private Formula makeFunctionCall(CFunctionCallEdge edge,
    ...
    List<CExpression> actualParams = edge.getArguments();//array====[0]:CIdExpression-->name:w  type:CDummyType  declaration-->type:CArrayType   [1]:CUnaryExpression CPointerType
    ...
    if (fn.getFunctionDefinition().getType().takesVarArgs()) {
      if (formalParams.size() > actualParams.size()) { throw new UnrecognizedCCodeException(
          "Number of parameters on function call does " +
  ...
  public Formula makePredicate(CExpression exp, CFAEdge edge, String function, SSAMapBuilder ssa)
  ...
  private boolean isMemoryLocation(CAstNode exp) {
    ...
    if (exp instanceof CFunctionCall) {
      ...
      if (fn instanceof CIdExpression) {
        ...
        if (memoryAllocationFunctions.contains(functionName)) { return true; }
      }

      // explicit heap/stack address?
    } else if (exp instanceof CUnaryExpression
        && ((CUnaryExpression) exp).getOperator() == UnaryOperator.AMPER) { return true; }
  ...
  private static boolean maybePointer(CType type, String varName, SSAMapBuilder ssa) {
    if (type != null && isStaticallyDeclaredPointer(type)) { return true; }
  ...
  private static CExpression removeCast(CExpression exp) {
    if (exp instanceof CCastExpression) { return removeCast(((CCastExpression) exp).getOperand()); }
    return exp;
  }
  ...
  private class ExpressionToFormulaVisitor extends DefaultCExpressionVisitor<Formula, UnrecognizedCCodeException> {
    ...
    protected Formula visitDefault(CExpression exp)
      ...
      //龙加的
      if (exp instanceof CArraySubscriptExpression) {//如：(std.sco)[1]

        CArraySubscriptExpression lstru = (CArraySubscriptExpression) exp;
        if (lstru.getArrayExpression() instanceof CFieldReference) {
          CFieldReference fs = (CFieldReference) lstru.getArrayExpression();
          if (fs.getFieldOwner() instanceof CIdExpression) {
            CIdExpression idexp = (CIdExpression) fs.getFieldOwner();
            if (idexp.getDeclaration() instanceof CVariableDeclaration) {
              CVariableDeclaration vardec = (CVariableDeclaration) idexp.getDeclaration();
              boolean isglobalflag = vardec.isGlobal();
              if (isglobalflag) {
                myfunction = "Global";
              }
            }
      ...
      if (exp instanceof CUnaryExpression) { return makeVariable(
          scoped(exprToVarName(((CUnaryExpression) exp).getOperand()), myfunction), ssa); }
      return makeVariable(scoped(exprToVarName(exp), myfunction), ssa);
    }
  ...
  private class ExpressionToFormulaVisitorPointers extends
    ...
    public Formula visit(CUnaryExpression exp)
      ...
      case STAR:
        if (opExp instanceof CIdExpression) { return makePointerVariable((CIdExpression) opExp, function, ssa); }
        //龙加入CBinaryExpression情况，如*（w+1）
        /*if(opExp instanceof CBinaryExpression){
         // Formula r=buildLvalueTerm(opExp,edge,function, ssa,constraints);

        return super.visit(opExp);
        }*/
        //$FALL-THROUGH$
      default:
        return super.visit(exp);
    ...
    private Formula makeAddressVariable(CUnaryExpression exp, String function)
      ...
      if (op != UnaryOperator.AMPER || !(operand instanceof CIdExpression)) { return super.visitDefault(exp); }
  ...
  private class StatementToFormulaVisitor extends RightHandSideToFormulaVisitor implements
      CStatementVisitor<Formula, UnrecognizedCCodeException> {
    ...
    public Formula visit(CAssignment assignment) throws UnrecognizedCCodeException {
      Formula rightVariable = assignment.getRightHandSide().accept(this);//1.0
      Formula r = toNumericFormula(rightVariable);//1.0
      //龙加入结构体中数组方法=====全局===方法名
      String myfunction = function;
      /*if(assignment.getLeftHandSide() instanceof CArraySubscriptExpression){//如：(std.sco)[1]

        CArraySubscriptExpression lstru=(CArraySubscriptExpression)assignment.getLeftHandSide();
       if(lstru.getArrayExpression() instanceof CFieldReference){
         CFieldReference fs=(CFieldReference)lstru.getArrayExpression();
         if(fs.getFieldOwner() instanceof CIdExpression){
           CIdExpression idexp=(CIdExpression)fs.getFieldOwner();
      ...
      Formula l = buildLvalueTerm(assignment.getLeftHandSide(), edge, myfunction, ssa, constraints);//|set::b+0@2|==（CArraySubscriptExpression,(std.sco)[1] = 1,set,）
      return fmgr.makeAssignment(l, r);
    }

    @Override
    public Formula visit(CExpressionAssignmentStatement pIastExpressionAssignmentStatement)
        throws UnrecognizedCCodeException {
      return visit((CAssignment) pIastExpressionAssignmentStatement);
    }

    @Override
    public Formula visit(CFunctionCallAssignmentStatement pIastFunctionCallAssignmentStatement)
        throws UnrecognizedCCodeException {
  ...
  private class StatementToFormulaVisitorPointers extends StatementToFormulaVisitor {
    ...
    private Formula handleIndirectAssignment(CAssignment pAssignment)
      ...
      CExpression lOperand = removeCast(l.getOperand());//CIdExpression----CBinaryExpression
      if ((!(lOperand instanceof CIdExpression))) {//&&(!(lOperand instanceof CBinaryExpression))
        // TODO: *(a + 2) = b
        return super.visit(pAssignment);
        //龙改动：：：：==========================
        /*CRightHandSide r = pAssignment.getRightHandSide();
        System.out.println(r.toASTString());
        System.out.println(lOperand.toASTString());
      ...
      //龙=============加入的if----CBinaryExpression
      //用于处理*(a + 2) = b情况
      /*   if(lOperand instanceof CBinaryExpression){
           CRightHandSide r = pAssignment.getRightHandSide();

             String lVarName=null;
             //String leftNo=null;
           int noint=0;
           CBinaryExpression btoid=(CBinaryExpression)lOperand;
          while( btoid.getOperand1() instanceof CBinaryExpression){
             btoid=(CBinaryExpression)(btoid.getOperand1());
           }
          if(btoid.getOperand1() instanceof CIdExpression)
          lVarName = scopedIfNecessary(this.edge,(CIdExpression)(btoid.getOperand1()), function);
           Formula lVar = makeVariable(lVarName, ssa);
           ...
           Formula rightVariable = pAssignment.getRightHandSide().accept(this);//6.0
           rightVariable = toNumericFormula(rightVariable);//6.0
           Formula lPtrVar = buildLvalueTerm(pAssignment.getLeftHandSide(), edge, function, ssa, constraints);//|__content_of__set::m__at__2__end@2|
           String var = scoped(exprToVarName(pAssignment.getLeftHandSide()), function);//set::*(b+1)
           System.out.println(lPtrVar.toString());//|set::b+1@3|
          String llstr= lPtrVar.toString();

         llstr= llstr.substring(llstr.indexOf("|")+1, llstr.indexOf("@"));

           Formula assignments = fmgr.makeAssignment(lPtrVar, rightVariable);

           updateAllPointers(lVarName, lVar, rVarName, rightVariable);//(= |__content_of__set::m__at__2__end@4| 6.0)

           boolean doDeepUpdate = (r instanceof CIdExpression);
           //用自己的方法====龙
           myUpdateAllMemoryLocations( llstr,lVarName, rPtrVar, rightVariable, doDeepUpdate);///constraints发生改变=====加上了：ite
           return assignments;
         }*/
      ...
      String lVarName = scopedIfNecessary((CIdExpression) lOperand, function);//set::m
      Formula lVar = makeVariable(lVarName, ssa);//|set::m@2|
      ...
      boolean doDeepUpdate = (r instanceof CIdExpression);//看是否是*b=q; q是个变量，需要深度更新
      updateAllMemoryLocations(lVarName, rPtrVar, rightVariable, doDeepUpdate);///constraints发生改变=====加上了：ite
    ...
    private void updateAllMemoryLocations(String lVarName, Formula rPtrVar, Formula rVar, boolean deepUpdate) {
      ...
      List<String> memAddresses = getAllMemoryLocationsFromSsaMap(ssa);//[__address_of__main::k]
      if (deepUpdate) {
        ...
        for (String memAddress : memAddresses) {//[__address_of__main::w, __address_of__main::k]
          String varName = getVariableNameFromMemoryAddress(memAddress);//main::w--------main::k

          if (!varName.equals(lVarName)) {

            Formula oldVar = makeVariable(varName, ssa);//|main::k@3|=====方法makeVariable不改变@值

            makeFreshIndex(varName, ssa);//方法makeFreshIndex改变@的值

            Formula newVar = makeVariable(varName, ssa);//|main::k@4|=======
            String newPtrVarName = makePointerMask(varName, ssa);//__content_of__main::k__at__4__end
            removeOldPointerVariablesFromSsaMap(newPtrVarName, ssa);

            Formula memAddressVar = makeVariable(memAddress, ssa);//|__address_of__main::k@1|

            Formula condition = fmgr.makeEqual(lVar, memAddressVar);//(= |set::m@2| |__address_of__main::k@1|)=====if的条件
            Formula equality = fmgr.makeAssignment(newVar, rVar);//(|main::k@4|,6.0)====>(= |main::k@4| 6.0)=======then语句
            Formula update = fmgr.makeAssignment(newVar, oldVar);//(= |main::k@4| |main::k@3|)=====================else语句
    ...
    /**
     * 本方法用于数组传递（通过地址传递到方法中改变数组的值）
     *@author 曹瑞龙
     */

    private void myUpdateAllMemoryLocations(String strno, String lVarName, Formula rPtrVar, Formula rVar,
        boolean deepUpdate) {
      ...
      if (deepUpdate) {
        ...
        for (String memAddress : memAddresses) {//[__address_of__main::w, __address_of__main::k]
          String varName = getVariableNameFromMemoryAddress(memAddress);//main::w--------main::k
          //strno= (String) strno.subSequence(strno.indexOf("::"),strno.length());
          varName = strno.replaceAll(lVarName, varName);

          /*if(!(strno==null)){


            varName=varName+"["+strno+"]";
            CRLGlobal.getInstance().nostr=null;
    ...
    private Formula handleDirectAssignment(CAssignment assignment)
      ...
      Operators.isLeft=true;
      Formula leftVariable = buildLvalueTerm(assignment.getLeftHandSide(), edge, function, ssa, constraints);//|main::k@3|
      Operators.isLeft=false;
      Operators.curKeyVar="";
  ...
  private class LvalueVisitor extends
    ...
    public Formula visit(CIdExpression idExp) {
      ...
      if(Operators.isLeft && KeyVar.keyVarSet.contains(var))//yangkai
      {
        Operators.curKeyVar=var;
      }
  ...
  private class LvalueVisitorPointers extends LvalueVisitor {
    ...
    private Formula getPointerFormula(CExpression pExp) {
      ...
      if (exp instanceof CIdExpression) {
        ...
        String ptrVarName = makePointerVariableName(ptrId, function, ssa);//__content_of__set::m__at__2__end
        makeFreshIndex(ptrVarName, ssa);
