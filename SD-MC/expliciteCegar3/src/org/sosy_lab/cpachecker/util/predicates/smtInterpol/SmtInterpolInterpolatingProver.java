...
public class SmtInterpolInterpolatingProver implements InterpolatingTheoremProver<Term> {
    ...
    public Term addFormula(Formula f) {
      ...
      //Term t = SmtInterpolFormulaManager.getTerm(f);
      Term t = ((SmtInterpolFormula)f).getTerm();
      //System.out.println(t.getTheory());
      String termName = prefix + counter++;
      Term annotatedTerm = env.annotate(t, new Annotation(":named", termName));
      //System.out.println("annotatedTerm=>"+annotatedTerm.getTheory().getLogic());
    ...
    @Override
    public int isUnsat1(){
        if(env.checkSat() == LBool.UNSAT)
          return 1;
        else if(env.checkSat() ==LBool.SAT)
          return 0;
        return -1;
    }
    @Override
    public Formula getInterpolant(List<Term> formulasOfA) {
        ...
        Term[] itp = env.getInterpolants(new Term[] {termA, termB});//得到了插值公式
        assert itp.length == 1; // 2 groups -> 1 interpolant
        //System.out.println("termB=>"+termB);
        //System.out.println("itp[0]=>"+itp[0]);
    ...
    public PathFormula getFormula(PathFormula oldPathFormula,CFAEdge edge){

      return oldPathFormula;
